...
import {
  IcDownload, IcEyeOff, IcFolder, IcHeart, IcImages, IcLayers, IcPin, IcPlus, IcRefresh, IcSearch, IcShare, IcX,
} from '../lib/icons';
...
export function GalleriesPage() {
  ...
  const load = useCallback(async () => {
    const [m, f, g] = await Promise.all([
      api.get<{ items: Photo[] }>(`/api/media?folder=${encodeURIComponent(folder)}&q=${encodeURIComponent(dq)}`),
    ...
    <div className="page">
      ...
      <div className="row" style={{ alignItems: 'flex-start', gap: 22 }}>
        <aside style={{ width: 230, flexShrink: 0, position: 'sticky', top: 'calc(var(--header-h) + 16px)' }} className="hide-mobile">
          <button className={`folder-item${folder === '' ? ' on' : ''}`} onClick={() => setFolder('')}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {photos === null ? (
            ...
            <VirtualGrid
              ...
              emptyNode={<Empty emoji="🗂️" title="No photos here" sub="Drop image folders into the library directory on the server, then hit Rescan." />}
            />
          )}
        </div>
      </div>
      ...
      {user && (
        <SelectionBar
          ...
          actions={[
            ...
            { key: 'pin', label: 'Pin', icon: <IcPin size={15} />, onClick: () => batch({ pinned: true }, 'Pinned') },
            { key: 'fav', label: 'Favorite', icon: <IcHeart size={15} />, onClick: () => batch({ favorite: true }, 'Favorited') },
